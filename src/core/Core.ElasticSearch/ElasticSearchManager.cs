...
public class ElasticSearchManager : IElasticSearch
{
    ...
    public ElasticSearchManager(IConfiguration configuration)
    {
        ElasticSearchConfig settings =
            configuration.GetSection("ElasticSearchConfig").Get<ElasticSearchConfig>()
            ?? throw new InvalidOperationException("ElasticSearchConfig is missing.");
    ...
    public async Task<IElasticSearchResult> InsertManyAsync(string indexName, object[] items)
    {
        ...
        return new ElasticSearchResult(response.IsValid, message: response.IsValid ? "Success" : response.ServerError?.Error?.Reason ?? "Unknown error");
    }
