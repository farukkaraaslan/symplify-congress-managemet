...
window.Symplify.EventRooms.Update = (function ($) {
    'use strict';

    const selectors = {
        editButton: '.js-event-room-update-button',
        modal: '#updateEventRoomModal',
        form: '#updateEventRoomForm',
    ...
    function init() {
        ...
        $(document)
            .off('click.event-roomsUpdate', selectors.editButton)
            .on('click.event-roomsUpdate', selectors.editButton, load);

        $(document)
            .off('submit.event-roomsUpdate', selectors.form)
            .on('submit.event-roomsUpdate', selectors.form, submitForm);

        $(document)
            .off('shown.bs.modal.event-roomsUpdate', selectors.modal)
            .on('shown.bs.modal.event-roomsUpdate', selectors.modal, function () {
                initializeValidation($(selectors.form));
                bindStatusSwitch($(selectors.form));
            });
    }
        ...
        postForm($form)
            .done(function (response) {
                ...
                if (window.Symplify.EventRooms.Table) {
                    window.Symplify.EventRooms.Table.reload(false);
                }
    ...
    function resolveGetForUpdateUrl() {
        ...
        return window.Symplify.EventRooms.urls?.getForUpdate || null;
    }
