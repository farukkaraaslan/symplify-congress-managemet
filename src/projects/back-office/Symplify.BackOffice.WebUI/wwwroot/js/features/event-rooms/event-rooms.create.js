...
window.Symplify.EventRooms.Create = (function ($) {
    'use strict';

    const selectors = {
        modal: '#createEventRoomModal',
        form: '#createEventRoomForm'
    };
    ...
    function init() {
        ...
        $(document)
            .off('submit.event-roomsCreate', selectors.form)
            .on('submit.event-roomsCreate', selectors.form, submitForm);

        $(document)
            .off('shown.bs.modal.event-roomsCreate', selectors.modal)
            .on('shown.bs.modal.event-roomsCreate', selectors.modal, function () {
                initializeValidation($(selectors.form));
                bindStatusSwitch($(selectors.form));
            });

        $(document)
            .off('hidden.bs.modal.event-roomsCreate', selectors.modal)
            .on('hidden.bs.modal.event-roomsCreate', selectors.modal, resetForm);
    }
        ...
        postForm($form)
            .done(function (response) {
                ...
                if (window.Symplify.EventRooms.Table) {
                    window.Symplify.EventRooms.Table.reload(true);
                }
