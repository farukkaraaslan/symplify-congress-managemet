...
window.Symplify.EventRooms.Table = (function ($) {
    'use strict';

    let table;

    const selectors = {
        table: '#eventRoomsTable',
    ...
    function init() {
        ...
        $table
            .off('order.dt.eventroomsTable search.dt.eventroomsTable page.dt.eventroomsTable draw.dt.eventroomsTable')
            .on('order.dt.eventroomsTable search.dt.eventroomsTable page.dt.eventroomsTable draw.dt.eventroomsTable', function () {
    ...
    function renderActions(id) {
        ...
        return '' +
            '<div class="d-flex align-items-center justify-content-end gap-2">' +
                '<button type="button" class="w-40-px h-40-px bg-primary-50 text-primary-600 rounded-circle d-inline-flex align-items-center justify-content-center js-lookup-update-button js-event-room-update-button" data-id="' + escapeHtml(id) + '" title="' + escapeHtml(editText) + '" aria-label="' + escapeHtml(editText) + '">' +
                    '<i class="ri-pencil-line"></i>' +
                '</button>' +
                '<button type="button" class="w-40-px h-40-px bg-danger-50 text-danger-600 rounded-circle d-inline-flex align-items-center justify-content-center js-lookup-delete-button js-event-room-delete-button" data-id="' + escapeHtml(id) + '" title="' + escapeHtml(deleteText) + '" aria-label="' + escapeHtml(deleteText) + '">' +
    ...
    function getTexts() {
        return window.Symplify.EventRooms.texts ||
