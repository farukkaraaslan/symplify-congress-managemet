...
window.Symplify.CongressSliders = window.Symplify.CongressSliders || {};

window.Symplify.CongressSliders.Index = (function ($) {
    'use strict';

    const selectors = {
        panel: '#congressSliderPanel',
        table: '#congressSlidersTable',
        modalContainer: '#congressSliderModalContainer',
        createButton: '#openCreateSliderModalButton',
        createForm: '#createCongressSliderForm',
        updateForm: '#updateCongressSliderForm',
        imageInput: '.js-slider-image-input',
        imageName: '.js-slider-image-file-name',
        dropzone: '[data-symplify-dropzone], .js-slider-image-dropzone',
        dragHandle: '.js-slider-drag-handle'
    };
    ...
    function initializeTable() {
        ...
        if (!$.fn.DataTable) {
            console.error('DataTables plugin bulunamadı. Congress slider tablosu başlatılamadı.');
            return;
        }
        ...
        $table
            .off('order.dt.congressSliders search.dt.congressSliders page.dt.congressSliders draw.dt.congressSliders')
            .on('order.dt.congressSliders search.dt.congressSliders page.dt.congressSliders draw.dt.congressSliders', function () {
    ...
    function bindEvents() {
        $(document)
            .off('click.congressSlidersCreate', selectors.createButton)
            .on('click.congressSlidersCreate', selectors.createButton, openCreateModal);

        $(document)
            .off('click.congressSlidersEdit', '.js-edit-congress-slider')
            .on('click.congressSlidersEdit', '.js-edit-congress-slider', openUpdateModal);

        $(document)
            .off('click.congressSlidersDelete', '.js-delete-congress-slider')
            .on('click.congressSlidersDelete', '.js-delete-congress-slider', deleteSlider);

        $(document)
            .off('submit.congressSlidersCreate', selectors.createForm)
            .on('submit.congressSlidersCreate', selectors.createForm, submitForm);

        $(document)
            .off('submit.congressSlidersUpdate', selectors.updateForm)
            .on('submit.congressSlidersUpdate', selectors.updateForm, submitForm);

        $(document)
            .off('change.congressSliderImage', selectors.imageInput)
            .on('change.congressSliderImage', selectors.imageInput, updateSelectedFileName);
    }
    ...
    function openCreateModal() {
        ...
        $.get(url)
            .done(function (html) {
                showModalHtml(html, '#createSliderModal');
            })
            .fail(function (xhr) {
                showError(xhr);
            });
    ...
    function openUpdateModal() {
        ...
        $.get($panel.data('edit-modal-url'), {
            ...
            .done(function (html) {
                showModalHtml(html, '#updateSliderModal');
            })
            .fail(function (xhr) {
                showError(xhr);
            });
    ...
    function deleteSlider() {
        ...
        confirmAction({
            ...
            text: text('deleteConfirmText', 'Bu slider silinecek.'),
    ...
    function initializeModal($modal) {
        ...
        if (window.Symplify.Dropzone && typeof window.Symplify.Dropzone.initAll === 'function') {
            window.Symplify.Dropzone.initAll($modal, {
                ...
                maxSizeMb: 5,
                invalidFileText: text('invalidImageFile', 'Sadece JPG, PNG veya WEBP formatında görsel seçebilirsiniz.'),
                fileTooLargeText: text('imageTooLarge', 'Görsel maksimum 5 MB olabilir.'),
                selectedText: text('selectedImage', 'Seçilen görsel'),
    ...
    function updateSelectedFileName() {
        ...
        const fileName = input.files && input.files.length
            ? input.files[0].name
            : text('imageHelp', 'Web banner ölçüsünde görsel önerilir.');

        $(input).closest('.col-12').find(selectors.imageName).text(fileName);
    }
    ...
    function renderDragHandle() {
        ...
        return '' +
            '<span role="button" tabindex="0" class="d-inline-flex align-items-center justify-content-center text-neutral-500 js-slider-drag-handle" title="' + escapeHtml(label) + '" aria-label="' + escapeHtml(label) + '">' +
    ...
    function renderOrder(data) {
        ...
        return '<span class="fw-medium text-secondary-light js-slider-order-value">' + escapeHtml(value) + '</span>';
    }
    ...
    function renderImage(data) {
        ...
        return '<img alt="Slider" class="w-100-px h-60-px rounded-2 object-fit-cover border" src="' + escapeHtml(data) + '" />';
    }
    ...
    function renderSubtitle(data) {
        ...
        return '<span class="text-neutral-500">' + escapeHtml(truncate(subtitle, 80)) + '</span>';
    }
    ...
    function renderActions(data, type, row) {
        ...
        return '' +
            ...
            '<button type="button" class="btn btn-primary-100 text-primary-600 radius-8 px-12 py-8 d-flex align-items-center justify-content-center w-40-px h-40-px js-edit-congress-slider" data-id="' + escapeHtml(id) + '" aria-label="' + escapeHtml(text('edit', 'Düzenle')) + '">' +
            '<i class="ri-edit-line"></i>' +
            '</button>' +
            '<button type="button" class="btn btn-danger-100 text-danger-600 radius-8 px-12 py-8 d-flex align-items-center justify-content-center w-40-px h-40-px js-delete-congress-slider" data-id="' + escapeHtml(id) + '" aria-label="' + escapeHtml(text('delete', 'Sil')) + '">' +
    ...
    function initializeNativeReorder() {
        ...
        $tbody.on('dragstart.sliderNativeReorder', selectors.dragHandle, function (event) {
            if (!isReorderAllowed()) {
                event.preventDefault();
                showReorderNotAllowedMessage();
                return false;
        ...
        $tbody.on('dragover.sliderNativeReorder', 'tr[data-id]', function (event) {
        ...
        $tbody.on('drop.sliderNativeReorder', 'tr[data-id]', function (event) {
            event.preventDefault();
        });

        $tbody.on('dragend.sliderNativeReorder', selectors.dragHandle, function () {
            if (draggedRow) {
    ...
    function updateVisibleRowNumbers() {
        ...
        $(table.table().body())
            ...
            .each(function (index) {
                ...
                $(this).find('td').eq(2).find('.js-slider-order-value').text(visibleNumber);
            });
    }
    ...
    function postForm($form) {
        if (window.Symplify.Ajax && typeof window.Symplify.Ajax.postForm === 'function') {
            return window.Symplify.Ajax.postForm($form, { multipart: true });
        }
    ...
    function text(key, fallback) {
        if (typeof window.Symplify.t === 'function') {
            return window.Symplify.t('BackOffice.CongressSliders.Js.' + key, fallback);
        }
    ...
    function ensureReorderStyles() {
        if (document.getElementById('symplify-slider-reorder-styles')) {
            return;
        }

        const style = document.createElement('style');
        style.id = 'symplify-slider-reorder-styles';
        style.textContent = '' +
            ...
            '.js-slider-drag-handle{cursor:grab;min-width:24px;}' +
            '.js-slider-drag-handle:active{cursor:grabbing;}' +
            '.js-slider-drag-handle.opacity-50{cursor:not-allowed!important;}';
...
$(function () {
    window.Symplify.CongressSliders.Index.init();
});