...
window.Symplify.TransactionStatusPhases.Table = (function ($) {
    'use strict';

    let table;
    const selectors = { table: '#transactionStatusPhasesTable', dragHandle: '.js-lookup-drag-handle' };
    ...
    function init() {
        ...
        table = $table.DataTable({
            ...
            ajax: {
                ...
                data: function (data) { data.culture = getCurrentCulture(); return data; },
                error: showError
            },
            columns: [
            ...
            createdRow: function (row, data) { $(row).attr('data-id', data?.id || '').attr('data-order', data?.order || ''); },
            drawCallback: function () { initializeReorder(); updateDragHandleState(); }
        });

        $table.data('symplify-feature-table-initialized', true);
        $table.off('order.dt.transactionStatusPhases search.dt.transactionStatusPhases page.dt.transactionStatusPhases draw.dt.transactionStatusPhases')
            .on('order.dt.transactionStatusPhases search.dt.transactionStatusPhases page.dt.transactionStatusPhases draw.dt.transactionStatusPhases', function () { window.setTimeout(function () { initializeReorder(); updateDragHandleState(); }, 0); });
    }
    ...
    function renderActions(id) { const texts = getTexts(); const editText = texts.edit || 'Düzenle'; const deleteText = texts.delete || 'Sil'; return '<div class="d-flex align-items-center justify-content-end gap-2"><button type="button" class="w-40-px h-40-px bg-primary-50 text-primary-600 rounded-circle d-inline-flex align-items-center justify-content-center js-transaction-status-phase-update-button" data-id="' + escapeHtml(id) + '" title="' + escapeHtml(editText) + '"><i class="ri-pencil-line"></i></button><button type="button" class="w-40-px h-40-px bg-danger-50 text-danger-600 rounded-circle d-inline-flex align-items-center justify-content-center js-transaction-status-phase-delete-button" data-id="' + escapeHtml(id) + '" title="' + escapeHtml(deleteText) + '"><i class="ri-delete-bin-line"></i></button></div>'; }
    ...
    function initializeSortableReorder() {
        ...
        $tbody.sortable({
            ...
            start: function (event, ui) { if (!isReorderAllowed()) { $(this).sortable('cancel'); showReorderNotAllowedMessage(); return; } ui.item.addClass('lookup-row-dragging'); ui.placeholder.html('<td colspan="8">&nbsp;</td>'); },
    ...
    function updateDragHandleState() { if (!table) return; const allowed = isReorderAllowed(); const $tbody = $(table.table().body()); const $handles = $tbody.find(selectors.dragHandle); if ($.fn.sortable && $tbody.data('ui-sortable')) $tbody.sortable(allowed ? 'enable' : 'disable'); $handles.attr('draggable', allowed ? 'true' : 'false').toggleClass('opacity-50', !allowed).css('cursor', allowed ? 'grab' : 'not-allowed'); }
    function isReorderAllowed() { const order = table.order(); const firstOrder = Array.isArray(order) && order.length > 0 ? order[0] : null; const isOrderAsc = firstOrder && Number(firstOrder[0]) === 2 && String(firstOrder[1] || '').toLowerCase() === 'asc'; return isOrderAsc && String(table.search() || '').trim().length === 0; }
    function showReorderNotAllowedMessage() { showError({ responseJSON: { message: getTexts().reorderNotAllowed || 'Sıralama yapmak için arama boş olmalı ve tablo Sıra kolonuna göre artan sıralanmalıdır.' } }); }
    function updateVisibleRowNumbers() { const pageInfo = table.page.info(); $(table.table().body()).find('tr[data-id]').each(function (index) { const visibleNumber = pageInfo.start + index + 1; $(this).find('td').eq(0).text(visibleNumber); $(this).find('td').eq(2).text(visibleNumber); }); }
    function persistReorder() { if (!isReorderAllowed()) { reload(false); return; } const reorderUrl = $(selectors.table).data('reorder-url'); if (!reorderUrl) { reload(false); return; } const pageInfo = table.page.info(); const items = []; $(table.table().body()).find('tr[data-id]').each(function (index) { const id = Number($(this).attr('data-id')); if (id > 0) items.push({ id: id, order: pageInfo.start + index + 1 }); }); if (!items.length) { reload(false); return; } $.ajax({ url: reorderUrl, type: 'POST', contentType: 'application/json; charset=utf-8', dataType: 'json', data: JSON.stringify({ items: items }), headers: getAjaxHeaders($(document)) }).done(function (response) { if (!response || response.success !== true) { showError(response); reload(false); return; } showSuccess(response.message || getTexts().reorderSuccess || getTexts().updated || 'Kayıt güncellendi.'); reload(false); }).fail(function (xhr) { showError(xhr); reload(false); }); }
    ...
    function getTexts() { return window.Symplify.TransactionStatusPhases.texts || window.Symplify.Lookup?.texts || window.Symplify.Texts || window.Symplify.texts || {}; }
