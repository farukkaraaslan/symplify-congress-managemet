...
window.Symplify.TransactionStatusPhases.Update = (function ($) {
    'use strict';

    const selectors = { editButton: '.js-transaction-status-phase-update-button', modal: '#updateTransactionStatusPhaseModal', form: '#updateTransactionStatusPhaseForm', table: '#transactionStatusPhasesTable' };

    function init() {
        initializeValidation($(selectors.form));
        removeAnyValidationSummary($(selectors.form));
        bindSwitches($(selectors.form));
        $(document).off('click.transactionstatusphaseUpdate', selectors.editButton).on('click.transactionstatusphaseUpdate', selectors.editButton, load);
        $(document).off('submit.transactionstatusphaseUpdate', selectors.form).on('submit.transactionstatusphaseUpdate', selectors.form, submitForm);
        $(document).off('shown.bs.modal.transactionstatusphaseUpdate', selectors.modal).on('shown.bs.modal.transactionstatusphaseUpdate', selectors.modal, function () { initializeValidation($(selectors.form)); removeAnyValidationSummary($(selectors.form)); bindSwitches($(selectors.form)); });
    }
    ...
    function submitForm(event) {
        ...
        if (hasJQueryValidation() && !$form.valid()) { removeAnyValidationSummary($form); focusFirstInvalidField($form); return; }
        ...
        postForm($form).done(function (response) {
            ...
            if (window.Symplify.TransactionStatusPhases.Table) window.Symplify.TransactionStatusPhases.Table.reload(false);
    ...
    function resolveGetForUpdateUrl() { return $(selectors.table).data('get-for-update-url') || window.Symplify.TransactionStatusPhases.urls?.getForUpdate || null; }
    ...
    function clearValidationErrors($form) { $form.find('.field-validation-error').removeClass('field-validation-error').addClass('field-validation-valid').text(''); $form.find('.input-validation-error, .is-invalid').removeClass('input-validation-error is-invalid'); removeAnyValidationSummary($form); }
    ...
    function removeAnyValidationSummary($form) { if (!$form || !$form.length) return; $form.find('.js-validation-summary, .js-lookup-validation-summary, [data-symplify-validation-summary], [data-valmsg-summary="true"], .validation-summary-errors, .validation-summary-valid').addClass('d-none').empty(); }
