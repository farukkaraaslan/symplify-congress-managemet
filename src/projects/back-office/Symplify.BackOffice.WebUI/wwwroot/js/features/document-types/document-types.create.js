...
window.Symplify.DocumentTypes.Create = (function ($) {
    'use strict';

    const selectors = {
        modal: '#createDocumentTypeModal',
    ...
    function init() {
        ...
        $(document)
            .off('submit.document-typesCreate', selectors.form)
            .on('submit.document-typesCreate', selectors.form, submitForm);

        $(document)
            .off('shown.bs.modal.document-typesCreate', selectors.modal)
            .on('shown.bs.modal.document-typesCreate', selectors.modal, function () {
                initializeValidation($(selectors.form));
                bindStatusSwitch($(selectors.form));
            });

        $(document)
            .off('hidden.bs.modal.document-typesCreate', selectors.modal)
            .on('hidden.bs.modal.document-typesCreate', selectors.modal, resetForm);
    }
        ...
        postForm($form)
            .done(function (response) {
                ...
                if (window.Symplify.DocumentTypes.Table) {
                    window.Symplify.DocumentTypes.Table.reload(true);
                }
