...
window.Symplify.Titles.Create = (function ($) {
    'use strict';

    const selectors = {
        modal: '#createTitleModal',
    ...
    function init() {
        ...
        $(document)
            .off('submit.titlesCreate', selectors.form)
            .on('submit.titlesCreate', selectors.form, submitForm);

        $(document)
            .off('shown.bs.modal.titlesCreate', selectors.modal)
            .on('shown.bs.modal.titlesCreate', selectors.modal, function () {
                initializeValidation($(selectors.form));
                bindStatusSwitch($(selectors.form));
            });

        $(document)
            .off('hidden.bs.modal.titlesCreate', selectors.modal)
            .on('hidden.bs.modal.titlesCreate', selectors.modal, resetForm);
    }
        ...
        postForm($form)
            .done(function (response) {
                ...
                if (window.Symplify.Titles.Table) {
                    window.Symplify.Titles.Table.reload(true);
                }
