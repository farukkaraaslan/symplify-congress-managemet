...
window.Symplify.Titles.Update = (function ($) {
    'use strict';

    const selectors = {
        editButton: '.js-title-update-button',
    ...
    function init() {
        ...
        $(document)
            .off('click.titlesUpdate', selectors.editButton)
            .on('click.titlesUpdate', selectors.editButton, load);

        $(document)
            .off('submit.titlesUpdate', selectors.form)
            .on('submit.titlesUpdate', selectors.form, submitForm);

        $(document)
            .off('shown.bs.modal.titlesUpdate', selectors.modal)
            .on('shown.bs.modal.titlesUpdate', selectors.modal, function () {
                initializeValidation($(selectors.form));
                bindStatusSwitch($(selectors.form));
            });
    }
        ...
        postForm($form)
            .done(function (response) {
                ...
                if (window.Symplify.Titles.Table) {
                    window.Symplify.Titles.Table.reload(false);
                }
    ...
    function resolveGetForUpdateUrl() {
        ...
        return window.Symplify.Titles.urls?.getForUpdate || null;
    }
