...
window.Symplify.TransactionStatuses.Create = (function ($) {
    'use strict';

    const selectors = { modal: '#createTransactionStatusModal', form: '#createTransactionStatusForm' };

    function init() {
        initializeValidation($(selectors.form));
        bindSwitches($(selectors.form));

        $(document).off('submit.transactionstatusCreate', selectors.form).on('submit.transactionstatusCreate', selectors.form, submitForm);
        $(document).off('shown.bs.modal.transactionstatusCreate', selectors.modal).on('shown.bs.modal.transactionstatusCreate', selectors.modal, function () {
            initializeValidation($(selectors.form));
            bindSwitches($(selectors.form));
        });
        $(document).off('hidden.bs.modal.transactionstatusCreate', selectors.modal).on('hidden.bs.modal.transactionstatusCreate', selectors.modal, resetForm);
    }
    ...
    function submitForm(event) {
        ...
        postForm($form).done(function (response) {
            ...
            resetForm();
            if (window.Symplify.TransactionStatuses.Table) window.Symplify.TransactionStatuses.Table.reload(true);
    ...
    function resetForm() {
        ...
        const selectedPhase = $('.js-transaction-status-phase-filter').val(); if (selectedPhase) $form.find('[name="TransactionStatusPhaseId"]').val(selectedPhase); else $form.find('[name="TransactionStatusPhaseId"] option:eq(1)').prop('selected', true);
        clearValidationErrors($form);
        showFirstTab($form);
    }
