...
window.Symplify.CongressSections = window.Symplify.CongressSections || {};

window.Symplify.CongressSections.Index = (function ($) {
    'use strict';

    const selectors = {
        panel: '#congressSectionPanel',
        table: '#congressSectionsTable',
        modalContainer: '#congressSectionModalContainer',
        createButton: '#openCreateSectionModalButton',
        createForm: '#createCongressSectionForm',
        updateForm: '#updateCongressSectionForm',
        dragHandle: '.js-section-drag-handle',
    ...
    function initializeTable() {
        ...
        if (!$.fn.DataTable) {
            console.error('DataTables plugin bulunamadı. Congress section tablosu başlatılamadı.');
            return;
        }
        ...
        $table
            .off('order.dt.congressSections search.dt.congressSections page.dt.congressSections draw.dt.congressSections')
            .on('order.dt.congressSections search.dt.congressSections page.dt.congressSections draw.dt.congressSections', function () {
    ...
    function bindEvents() {
        $(document)
            .off('click.congressSectionsCreate', selectors.createButton)
            .on('click.congressSectionsCreate', selectors.createButton, openCreateModal);

        $(document)
            .off('click.congressSectionsEdit', '.js-edit-congress-section')
            .on('click.congressSectionsEdit', '.js-edit-congress-section', openUpdateModal);

        $(document)
            .off('click.congressSectionsDelete', '.js-delete-congress-section')
            .on('click.congressSectionsDelete', '.js-delete-congress-section', deleteSection);

        $(document)
            .off('submit.congressSectionsCreate', selectors.createForm)
            .on('submit.congressSectionsCreate', selectors.createForm, submitForm);

        $(document)
            .off('submit.congressSectionsUpdate', selectors.updateForm)
            .on('submit.congressSectionsUpdate', selectors.updateForm, submitForm);
    }
    ...
    function openCreateModal() {
        ...
        $.get(url)
            .done(function (html) {
                showModalHtml(html, '#createSectionModal');
            })
            .fail(function (xhr) {
                showError(xhr);
            });
    ...
    function openUpdateModal() {
        ...
        $.get($panel.data('edit-modal-url'), {
            ...
            .done(function (html) {
                showModalHtml(html, '#updateSectionModal');
            })
            .fail(function (xhr) {
                showError(xhr);
            });
    ...
    function renderDragHandle() {
        ...
        return '' +
            '<span role="button" tabindex="0" class="d-inline-flex align-items-center justify-content-center text-neutral-500 js-section-drag-handle" title="' + escapeHtml(label) + '" aria-label="' + escapeHtml(label) + '">' +
    ...
    function renderOrder(data, type, row, meta) {
        ...
        return '<span class="fw-medium text-secondary-light js-section-order-value">' + escapeHtml(value) + '</span>';
    }

    function renderBindingKey(data) {
        if (!data) {
            return '<span class="text-neutral-400">-</span>';
        }

        return '<code class="text-sm">' + escapeHtml(data) + '</code>';
    }
    ...
    function renderContent(data) {
        const textValue = truncate(stripHtml(data), 120);
    ...
    function renderActions(data, type, row) {
        ...
        return '' +
            '<div class="d-flex align-items-center justify-content-end gap-2">' +
                '<button type="button" class="btn btn-primary-100 text-primary-600 radius-8 px-12 py-8 d-flex align-items-center justify-content-center w-40-px h-40-px js-edit-congress-section" data-id="' + escapeHtml(id) + '" aria-label="' + escapeHtml(text('edit', 'Düzenle')) + '">' +
                    '<i class="ri-edit-line"></i>' +
                '</button>' +
                '<button type="button" class="btn btn-danger-100 text-danger-600 radius-8 px-12 py-8 d-flex align-items-center justify-content-center w-40-px h-40-px js-delete-congress-section" data-id="' + escapeHtml(id) + '" aria-label="' + escapeHtml(text('delete', 'Sil')) + '">' +
    ...
    function initializeSortableReorder() {
        ...
        $tbody.off('.sectionNativeReorder');
    ...
    function initializeNativeReorder() {
        ...
        $tbody.on('dragstart.sectionNativeReorder', selectors.dragHandle, function (event) {
            if (!isReorderAllowed()) {
                event.preventDefault();
                showReorderNotAllowedMessage();
                return false;
        ...
        $tbody.on('dragover.sectionNativeReorder', 'tr[data-id]', function (event) {
        ...
        $tbody.on('drop.sectionNativeReorder', 'tr[data-id]', function (event) {
            event.preventDefault();
        });

        $tbody.on('dragend.sectionNativeReorder', selectors.dragHandle, function () {
            if (draggedRow) {
    ...
    function updateVisibleRowNumbers() {
        ...
        $(table.table().body())
            ...
            .each(function (index) {
                ...
                $(this).find('td').eq(2).find('.js-section-order-value').text(visibleNumber);
            });
    }
    ...
    function text(key, fallback) {
        if (typeof window.Symplify.t === 'function') {
            return window.Symplify.t('BackOffice.CongressSections.Js.' + key, fallback);
        }
    ...
    function ensureReorderStyles() {
        if (document.getElementById('symplify-section-reorder-styles')) {
            return;
        }

        const style = document.createElement('style');
        style.id = 'symplify-section-reorder-styles';
        style.textContent = '' +
            ...
            '.js-section-drag-handle{cursor:grab;min-width:24px;}' +
            '.js-section-drag-handle:active{cursor:grabbing;}' +
            '.js-section-drag-handle.opacity-50{cursor:not-allowed!important;}';
...
$(function () {
    window.Symplify.CongressSections.Index.init();
});
