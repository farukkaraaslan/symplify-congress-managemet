...
window.Symplify.Topics.Update = (function ($) {
    'use strict';

    const selectors = {
        editButton: '.js-topic-update-button',
        modal: '#updateTopicModal',
    ...
    function init() {
        ...
        $(document)
            .off('click.topicsUpdate', selectors.editButton)
            .on('click.topicsUpdate', selectors.editButton, load);

        $(document)
            .off('submit.topicsUpdate', selectors.form)
            .on('submit.topicsUpdate', selectors.form, submitForm);

        $(document)
            .off('shown.bs.modal.topicsUpdate', selectors.modal)
            .on('shown.bs.modal.topicsUpdate', selectors.modal, function () {
                initializeValidation($(selectors.form));
                bindStatusSwitch($(selectors.form));
            });
    }
        ...
        postForm($form)
            .done(function (response) {
                ...
                if (window.Symplify.Topics.Table) {
                    window.Symplify.Topics.Table.reload(false);
                }
    ...
    function resolveGetForUpdateUrl() {
        ...
        return window.Symplify.Topics.urls?.getForUpdate || null;
    }
