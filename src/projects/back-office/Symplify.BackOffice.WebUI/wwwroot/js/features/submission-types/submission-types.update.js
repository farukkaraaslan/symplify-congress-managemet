...
window.Symplify.SubmissionTypes.Update = (function ($) {
    'use strict';

    const selectors = {
        editButton: '.js-submission-type-update-button',
        modal: '#updateSubmissionTypeModal',
        form: '#updateSubmissionTypeForm',
    ...
    function init() {
        ...
        $(document)
            .off('click.submission-typesUpdate', selectors.editButton)
            .on('click.submission-typesUpdate', selectors.editButton, load);

        $(document)
            .off('submit.submission-typesUpdate', selectors.form)
            .on('submit.submission-typesUpdate', selectors.form, submitForm);

        $(document)
            .off('shown.bs.modal.submission-typesUpdate', selectors.modal)
            .on('shown.bs.modal.submission-typesUpdate', selectors.modal, function () {
                initializeValidation($(selectors.form));
                bindStatusSwitch($(selectors.form));
            });
    }
        ...
        postForm($form)
            .done(function (response) {
                ...
                if (window.Symplify.SubmissionTypes.Table) {
                    window.Symplify.SubmissionTypes.Table.reload(false);
                }
    ...
    function resolveGetForUpdateUrl() {
        ...
        return window.Symplify.SubmissionTypes.urls?.getForUpdate || null;
    }
