...
window.Symplify.EvaluationCriteria.Update = (function ($) {
    'use strict';

    const selectors = {
        editButton: '.js-evaluation-criterion-update-button',
        modal: '#updateEvaluationCriterionModal',
        form: '#updateEvaluationCriterionForm',
    ...
    function init() {
        ...
        $(document)
            .off('click.evaluation-criteriaUpdate', selectors.editButton)
            .on('click.evaluation-criteriaUpdate', selectors.editButton, load);

        $(document)
            .off('submit.evaluation-criteriaUpdate', selectors.form)
            .on('submit.evaluation-criteriaUpdate', selectors.form, submitForm);

        $(document)
            .off('shown.bs.modal.evaluation-criteriaUpdate', selectors.modal)
            .on('shown.bs.modal.evaluation-criteriaUpdate', selectors.modal, function () {
                initializeValidation($(selectors.form));
                bindStatusSwitch($(selectors.form));
            });
    }
        ...
        postForm($form)
            .done(function (response) {
                ...
                if (window.Symplify.EvaluationCriteria.Table) {
                    window.Symplify.EvaluationCriteria.Table.reload(false);
                }
    ...
    function fillForm(response) {
        ...
        $form.find('[name="Score"]').val(response.score || 10);
    ...
    function resolveGetForUpdateUrl() {
        ...
        return window.Symplify.EvaluationCriteria.urls?.getForUpdate || null;
    }
