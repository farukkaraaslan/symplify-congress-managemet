...
window.Symplify.EvaluationCriteria.Create = (function ($) {
    'use strict';

    const selectors = {
        modal: '#createEvaluationCriterionModal',
        form: '#createEvaluationCriterionForm'
    };
    ...
    function init() {
        ...
        $(document)
            .off('submit.evaluation-criteriaCreate', selectors.form)
            .on('submit.evaluation-criteriaCreate', selectors.form, submitForm);

        $(document)
            .off('shown.bs.modal.evaluation-criteriaCreate', selectors.modal)
            .on('shown.bs.modal.evaluation-criteriaCreate', selectors.modal, function () {
                initializeValidation($(selectors.form));
                bindStatusSwitch($(selectors.form));
            });

        $(document)
            .off('hidden.bs.modal.evaluation-criteriaCreate', selectors.modal)
            .on('hidden.bs.modal.evaluation-criteriaCreate', selectors.modal, resetForm);
    }
        ...
        postForm($form)
            .done(function (response) {
                ...
                if (window.Symplify.EvaluationCriteria.Table) {
                    window.Symplify.EvaluationCriteria.Table.reload(true);
                }
