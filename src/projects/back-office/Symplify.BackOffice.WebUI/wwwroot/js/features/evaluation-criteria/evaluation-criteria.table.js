...
window.Symplify.EvaluationCriteria.Table = (function ($) {
    'use strict';
    ...
    function init() {
        ...
        $table
            .off('order.dt.evaluationcriteriaTable search.dt.evaluationcriteriaTable page.dt.evaluationcriteriaTable draw.dt.evaluationcriteriaTable')
            .on('order.dt.evaluationcriteriaTable search.dt.evaluationcriteriaTable page.dt.evaluationcriteriaTable draw.dt.evaluationcriteriaTable', function () {
    ...
    function renderScore(value) {
        ...
        return '<span class="badge bg-primary-50 text-primary-600 rounded-pill px-12 py-6">' + escapeHtml(score > 0 ? score : 10) + '</span>';
    }
    ...
    function renderActions(id) {
        ...
        return '' +
            '<div class="d-flex align-items-center justify-content-end gap-2">' +
                '<button type="button" class="w-40-px h-40-px bg-primary-50 text-primary-600 rounded-circle d-inline-flex align-items-center justify-content-center js-lookup-update-button js-evaluation-criterion-update-button" data-id="' + escapeHtml(id) + '" title="' + escapeHtml(editText) + '" aria-label="' + escapeHtml(editText) + '">' +
                    '<i class="ri-pencil-line"></i>' +
                '</button>' +
                '<button type="button" class="w-40-px h-40-px bg-danger-50 text-danger-600 rounded-circle d-inline-flex align-items-center justify-content-center js-lookup-delete-button js-evaluation-criterion-delete-button" data-id="' + escapeHtml(id) + '" title="' + escapeHtml(deleteText) + '" aria-label="' + escapeHtml(deleteText) + '">' +
    ...
    function showReorderNotAllowedMessage() {
        ...
        window.Symplify.Ajax.showError({ message: message });
    }
    ...
    function getTexts() {
        return window.Symplify.EvaluationCriteria.texts ||
    ...
    function ensureExpectedHeader($table) {
        ...
        const headerHtml = headers.map(function (header, index) {
            const className = index === 7 ? ' class="text-end"' : '';
