...
window.Symplify.TransactionStatusTransitions.Create = (function ($) {
    'use strict';

    const selectors = {
        modal: '#createTransactionStatusTransitionModal',
        form: '#createTransactionStatusTransitionForm'
    };
    ...
    function init() {
        ...
        $(document)
            .off('submit.transactionStatusTransitionsCreate', selectors.form)
            .on('submit.transactionStatusTransitionsCreate', selectors.form, submitForm);

        $(document)
            .off('shown.bs.modal.transactionStatusTransitionsCreate', selectors.modal)
            .on('shown.bs.modal.transactionStatusTransitionsCreate', selectors.modal, function () {
                initializeValidation($(selectors.form));
                bindStatusSwitch($(selectors.form));
            });

        $(document)
            .off('hidden.bs.modal.transactionStatusTransitionsCreate', selectors.modal)
            .on('hidden.bs.modal.transactionStatusTransitionsCreate', selectors.modal, resetForm);
    }
        ...
        postForm($form)
            .done(function (response) {
                ...
                if (window.Symplify.TransactionStatusTransitions.Table) {
                    window.Symplify.TransactionStatusTransitions.Table.reload(true);
                }
    ...
    function resetForm() {
        ...
        $form[0].reset();
        $form.find('select[name="FromStatusId"]').val('');
        $form.find('select[name="ToStatusId"]').val('');
