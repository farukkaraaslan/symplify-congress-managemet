...
(function ($) {
  ...
  function collectSidebarOpenDropdownIds() {
    ...
    $root.find("li.dropdown.open, li.dropdown.dropdown-open").each(function () {
      const $item = $(this);
      const persistOpen = String(
        $item.attr("data-sidebar-persist-open") || "true"
      ).toLowerCase();

      // Some menu groups (for example Bulk Emails) must only stay open
      // while one of their own pages is active. Do not persist those groups.
      if (persistOpen === "false") return;
  ...
  function applySidebarOpenState(openDropdownIds) {
    ...
    openDropdownIds.forEach((id) => {
      ...
      if (persistOpen === "false") {
        ...
        $item.children(".sidebar-submenu").hide();
        return;
      }
  ...
  $document.on("click", ".sidebar-menu .dropdown > a", function (e) {
    const $trigger = $(this);
    const $item = $trigger.parent(".dropdown");
    ...
    const hasActiveChild =
      $submenu.find("a.active-page, li.active-page").length > 0;

    const isCurrentlyVisible = $submenu.is(":visible");
    const willOpen = !isCurrentlyVisible;
    ...
    /*
     * Route-active dropdowns keep the standard blue parent state.
     * A non-active, non-persistent dropdown (Bulk Emails) may be expanded
     * only to expose its child links, but it must not look like the active
     * page while another page such as Video Presentations is selected.
     */
    if (persistOpen === "false" && !hasActiveChild) {
      $item.removeClass("dropdown-open open");
    ...
    $submenu.stop(true, true).slideToggle(200);
    $item.toggleClass("dropdown-open open", willOpen);
    $trigger.attr("aria-expanded", willOpen ? "true" : "false");
    saveSidebarState();
  });
  ...
  function activateSidebarMenu() {
    ...
    const currentUrl = toUrl(stripHash(window.location.href));
    if (!currentUrl) return;

    const currentFull = `${normalizePath(currentUrl.pathname)}${currentUrl.search}`;
    const currentPath = normalizePath(currentUrl.pathname);
    const $menuRoot = $("ul#sidebar-menu");

    // Dropdown tetikleyicileri href="#" kullandığı için tarayıcı bunları mevcut
    // sayfanın URL'si olarak çözer. Bunları aktif-link eşleştirmesine katarsak
    // Toplu E-Posta parent menüsü her sayfada active-page olur.
    const $links = $menuRoot.find("a").filter(function () {
      const $link = $(this);
      const rawHref = String($link.attr("href") || "").trim();
      const hasSubmenu = $link.siblings(".sidebar-submenu").length > 0;
    ...
    // Server-side veya önceki sayfadan kalan yanlış aktif sınıfları temizle.
    // Parent dropdown yalnızca kendi child route'u gerçekten aktifse açılır.
    $menuRoot.find("a, li").removeClass("active-page");

    let $activeItem = $links
      .filter(function () {
        const linkUrl = toUrl(stripHash(this.href));
        if (!linkUrl) return false;

        const linkFull = `${normalizePath(linkUrl.pathname)}${linkUrl.search}`;
        return linkFull.toLowerCase() === currentFull.toLowerCase();
      })
      .first();
    ...
    if (!$activeItem.length) {
      $activeItem = $links
        .filter(function () {
          const linkUrl = toUrl(stripHash(this.href));
          if (!linkUrl || linkUrl.search) return false;
