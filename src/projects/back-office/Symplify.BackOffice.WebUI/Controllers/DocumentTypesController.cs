...
using Symplify.BackOffice.Application.Features.DocumentTypes.Commands.Reorder;
using Symplify.BackOffice.Application.Features.DocumentTypes.Commands.Update;
using Symplify.BackOffice.Application.Features.DocumentTypes.Queries.GetForUpdate;
...
using Symplify.BackOffice.WebUI.Models.DocumentTypes;

namespace Symplify.BackOffice.WebUI.Controllers;

[Authorize]
[Route("{culture=tr-TR}/[controller]/[action]")]
public sealed class DocumentTypesController : Controller
{
    ...
    {
        DocumentTypesIndexViewModel model = await BuildIndexViewModelAsync(cancellationToken);
        return View(model);
    }
    ...
    public async Task<IActionResult> GetForUpdate(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            GetDocumentTypeForUpdateResponse response = await _mediator.Send(
                new GetDocumentTypeForUpdateQuery
                {
                    Id = id
                },
                cancellationToken);
    ...
    public async Task<IActionResult> Create(
        [FromForm] CreateDocumentTypeViewModel model,
        CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Update(
        [FromForm] UpdateDocumentTypeViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await _mediator.Send(
                new ReorderDocumentTypeCommand
                {
                    Items = request.Items
                        .Where(item => item.Id != Guid.Empty)
                        .Select(item => new ReorderDocumentTypeItemDto
                        {
                            Id = item.Id,
                            Order = item.Order
                        })
    ...
    private async Task<DocumentTypesIndexViewModel> BuildIndexViewModelAsync(CancellationToken cancellationToken)
    {
        ...
        return new DocumentTypesIndexViewModel
        {
            CreateDocumentType = new CreateDocumentTypeViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new CreateDocumentTypeTranslationViewModel
                {
            ...
            UpdateDocumentType = new UpdateDocumentTypeViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new UpdateDocumentTypeTranslationViewModel
                {
                    LanguageId = language.Id,
                    Culture = language.Culture,
                    LanguageName = language.Name,
    ...
    private void ValidateCreateModel(CreateDocumentTypeViewModel model)
    {
        ...
        CreateDocumentTypeTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];

        if (string.IsNullOrWhiteSpace(defaultTranslation.Name))
        {
            ModelState.AddModelError(
                $"Translations[{defaultLanguageIndex}].Name",
                GetText("BackOffice.DocumentTypes.Validation.NameRequired", "Ad alanı zorunludur."));
        }
    }

    private void ValidateUpdateModel(UpdateDocumentTypeViewModel model)
    {
        ...
        UpdateDocumentTypeTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<CreateDocumentTypeTranslationViewModel> translations)
    {
        return translations
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<UpdateDocumentTypeTranslationViewModel> translations)
    {
        return translations
            .GroupBy(translation => translation.LanguageId)
            .Select(group => group.First())
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        CreateDocumentTypeTranslationViewModel translation)
    {
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        UpdateDocumentTypeTranslationViewModel translation)
    {
        return new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
        {
            [NameField] = NormalizeText(translation.Name),
    ...
    private static bool HasAnyTranslationValue(CreateDocumentTypeTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }

    private static bool HasAnyTranslationValue(UpdateDocumentTypeTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }
