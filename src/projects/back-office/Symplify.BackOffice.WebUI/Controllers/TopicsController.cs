...
using Symplify.BackOffice.Application.Features.Topics.Commands.Reorder;
using Symplify.BackOffice.Application.Features.Topics.Commands.Update;
using Symplify.BackOffice.Application.Features.Topics.Queries.GetForUpdate;
using Symplify.BackOffice.Application.Features.Topics.Queries.GetList;
using Symplify.BackOffice.Application.Services.Localization;
using Symplify.BackOffice.WebUI.Localization;
using Symplify.BackOffice.WebUI.Models.Shared.DataTables;
using Symplify.BackOffice.WebUI.Models.Topics;
...
public sealed class TopicsController : Controller
{
    ...
    {
        TopicsIndexViewModel model = await BuildIndexViewModelAsync(cancellationToken);
        return View(model);
    }
    ...
    public async Task<IActionResult> GetForUpdate(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            GetTopicForUpdateResponse response = await _mediator.Send(
                new GetTopicForUpdateQuery
                {
                    Id = id
                },
                cancellationToken);
    ...
    public async Task<IActionResult> Create(
        [FromForm] CreateTopicViewModel model,
        CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Update(
        [FromForm] UpdateTopicViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            UpdateTopicCommand command = new()
            {
                Id = model.Id,
                Code = NormalizeText(model.Code),
                IsActive = model.IsActive,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await _mediator.Send(
                new ReorderTopicCommand
                {
                    Items = request.Items
                        .Where(item => item.Id != Guid.Empty)
                        .Select(item => new ReorderTopicItemDto
                        {
                            Id = item.Id,
                            Order = item.Order
                        })
    ...
    private async Task<TopicsIndexViewModel> BuildIndexViewModelAsync(CancellationToken cancellationToken)
    {
        ...
        return new TopicsIndexViewModel
        {
            CreateTopic = new CreateTopicViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new CreateTopicTranslationViewModel
                {
            ...
            UpdateTopic = new UpdateTopicViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new UpdateTopicTranslationViewModel
                {
                    LanguageId = language.Id,
                    Culture = language.Culture,
                    LanguageName = language.Name,
    ...
    private void ValidateCreateModel(CreateTopicViewModel model)
    {
        ...
        CreateTopicTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];

        if (string.IsNullOrWhiteSpace(defaultTranslation.Name))
        {
            ModelState.AddModelError(
                $"Translations[{defaultLanguageIndex}].Name",
                GetText("BackOffice.Topics.Validation.NameRequired", "Ad alanı zorunludur."));
        }
    }

    private void ValidateUpdateModel(UpdateTopicViewModel model)
    {
        ...
        UpdateTopicTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<CreateTopicTranslationViewModel> translations)
    {
        return translations
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<UpdateTopicTranslationViewModel> translations)
    {
        return translations
            .GroupBy(translation => translation.LanguageId)
            .Select(group => group.First())
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        CreateTopicTranslationViewModel translation)
    {
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        UpdateTopicTranslationViewModel translation)
    {
        return new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
        {
            [NameField] = NormalizeText(translation.Name),
    ...
    private static bool HasAnyTranslationValue(CreateTopicTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }

    private static bool HasAnyTranslationValue(UpdateTopicTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }
