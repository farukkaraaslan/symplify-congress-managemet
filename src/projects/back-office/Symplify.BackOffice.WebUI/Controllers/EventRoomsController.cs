...
using Symplify.BackOffice.Application.Features.EventRooms.Commands.Reorder;
using Symplify.BackOffice.Application.Features.EventRooms.Commands.Update;
using Symplify.BackOffice.Application.Features.EventRooms.Queries.GetForUpdate;
using Symplify.BackOffice.Application.Features.EventRooms.Queries.GetList;
using Symplify.BackOffice.Application.Services.Localization;
using Symplify.BackOffice.WebUI.Localization;
using Symplify.BackOffice.WebUI.Models.Shared.DataTables;
using Symplify.BackOffice.WebUI.Models.EventRooms;

namespace Symplify.BackOffice.WebUI.Controllers;

[Authorize]
[Route("{culture=tr-TR}/[controller]/[action]")]
public sealed class EventRoomsController : Controller
{
    ...
    {
        EventRoomsIndexViewModel model = await BuildIndexViewModelAsync(cancellationToken);
        return View(model);
    }
    ...
    public async Task<IActionResult> GetForUpdate(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            GetEventRoomForUpdateResponse response = await _mediator.Send(
                new GetEventRoomForUpdateQuery
                {
                    Id = id
                },
                cancellationToken);
    ...
    public async Task<IActionResult> Create(
        [FromForm] CreateEventRoomViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            CreateEventRoomCommand command = new()
            {
    ...
    public async Task<IActionResult> Update(
        [FromForm] UpdateEventRoomViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            UpdateEventRoomCommand command = new()
            {
                Id = model.Id,
                Code = NormalizeText(model.Code),
                IsActive = model.IsActive,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await _mediator.Send(
                new ReorderEventRoomCommand
                {
                    Items = request.Items
                        .Where(item => item.Id != Guid.Empty)
                        .Select(item => new ReorderEventRoomItemDto
                        {
                            Id = item.Id,
                            Order = item.Order
                        })
    ...
    private async Task<EventRoomsIndexViewModel> BuildIndexViewModelAsync(CancellationToken cancellationToken)
    {
        ...
        return new EventRoomsIndexViewModel
        {
            CreateEventRoom = new CreateEventRoomViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new CreateEventRoomTranslationViewModel
                {
            ...
            UpdateEventRoom = new UpdateEventRoomViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new UpdateEventRoomTranslationViewModel
                {
                    LanguageId = language.Id,
                    Culture = language.Culture,
                    LanguageName = language.Name,
    ...
    private void ValidateCreateModel(CreateEventRoomViewModel model)
    {
        ...
        CreateEventRoomTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];

        if (string.IsNullOrWhiteSpace(defaultTranslation.Name))
        {
            ModelState.AddModelError(
                $"Translations[{defaultLanguageIndex}].Name",
                GetText("BackOffice.EventRooms.Validation.NameRequired", "Ad alanı zorunludur."));
        }
    }

    private void ValidateUpdateModel(UpdateEventRoomViewModel model)
    {
        ...
        UpdateEventRoomTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<CreateEventRoomTranslationViewModel> translations)
    {
        return translations
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<UpdateEventRoomTranslationViewModel> translations)
    {
        return translations
            .GroupBy(translation => translation.LanguageId)
            .Select(group => group.First())
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        CreateEventRoomTranslationViewModel translation)
    {
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        UpdateEventRoomTranslationViewModel translation)
    {
        return new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
        {
            [NameField] = NormalizeText(translation.Name),
    ...
    private static bool HasAnyTranslationValue(CreateEventRoomTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }

    private static bool HasAnyTranslationValue(UpdateEventRoomTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }
