...
using Symplify.BackOffice.Application.Features.Titles.Commands.Reorder;
using Symplify.BackOffice.Application.Features.Titles.Commands.Update;
using Symplify.BackOffice.Application.Features.Titles.Queries.GetForUpdate;
...
using Symplify.BackOffice.WebUI.Models.Titles;

namespace Symplify.BackOffice.WebUI.Controllers;

[Authorize]
[Route("{culture=tr-TR}/[controller]/[action]")]
public sealed class TitlesController : Controller
{
    ...
    {
        TitlesIndexViewModel model = await BuildIndexViewModelAsync(cancellationToken);
        return View(model);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        DataTableQueryOptions tableOptions = DataTableQueryOptions.From(
            ...
            allowedSortColumns: new[] { "order", "name", "description", "isActive" });
    ...
    public async Task<IActionResult> GetForUpdate(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            GetTitleForUpdateResponse response = await _mediator.Send(
                new GetTitleForUpdateQuery
                {
                    Id = id
                },
                cancellationToken);
    ...
    public async Task<IActionResult> Create(
        [FromForm] CreateTitleViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            CreateTitleCommand command = new()
            {
    ...
    public async Task<IActionResult> Update(
        [FromForm] UpdateTitleViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            UpdateTitleCommand command = new()
            {
                Id = model.Id,
                Code = NormalizeText(model.Code),
                IsActive = model.IsActive,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await _mediator.Send(
                new ReorderTitleCommand
                {
                    Items = request.Items
                        .Where(item => item.Id != Guid.Empty)
                        .Select(item => new ReorderTitleItemDto
                        {
                            Id = item.Id,
                            Order = item.Order
                        })
    ...
    private async Task<TitlesIndexViewModel> BuildIndexViewModelAsync(CancellationToken cancellationToken)
    {
        ...
        return new TitlesIndexViewModel
        {
            CreateTitle = new CreateTitleViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new CreateTitleTranslationViewModel
                {
            ...
            UpdateTitle = new UpdateTitleViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new UpdateTitleTranslationViewModel
                {
                    LanguageId = language.Id,
                    Culture = language.Culture,
                    LanguageName = language.Name,
    ...
    private void ValidateCreateModel(CreateTitleViewModel model)
    {
        ...
        CreateTitleTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];

        if (string.IsNullOrWhiteSpace(defaultTranslation.Name))
        {
            ModelState.AddModelError(
                $"Translations[{defaultLanguageIndex}].Name",
                GetText("BackOffice.Titles.Validation.NameRequired", "Ad alanı zorunludur."));
        }
    }

    private void ValidateUpdateModel(UpdateTitleViewModel model)
    {
        ...
        UpdateTitleTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<CreateTitleTranslationViewModel> translations)
    {
        return translations
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<UpdateTitleTranslationViewModel> translations)
    {
        return translations
            .GroupBy(translation => translation.LanguageId)
            .Select(group => group.First())
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        CreateTitleTranslationViewModel translation)
    {
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        UpdateTitleTranslationViewModel translation)
    {
        return new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
        {
            [NameField] = NormalizeText(translation.Name),
    ...
    private static bool HasAnyTranslationValue(CreateTitleTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }

    private static bool HasAnyTranslationValue(UpdateTitleTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }
