...
using Symplify.BackOffice.Application.Features.WorkflowTemplates.Commands.Create;
using Symplify.BackOffice.Application.Features.WorkflowTemplates.Commands.Delete;
using Symplify.BackOffice.Application.Features.WorkflowTemplates.Commands.Update;
using Symplify.BackOffice.Application.Features.WorkflowTemplates.Queries.GetForUpdate;
using Symplify.BackOffice.Application.Features.WorkflowTemplates.Queries.GetList;
using Symplify.BackOffice.Application.Services.Localization;
using Symplify.BackOffice.WebUI.Localization;
using Symplify.BackOffice.WebUI.Models.Shared.DataTables;
using Symplify.BackOffice.WebUI.Models.WorkflowTemplates;

namespace Symplify.BackOffice.WebUI.Controllers;

[Authorize]
[Route("{culture=tr-TR}/[controller]/[action]")]
public sealed class WorkflowTemplatesController : Controller
{
    ...
    {
        WorkflowTemplatesIndexViewModel model = await BuildIndexViewModelAsync(cancellationToken);
        return View(model);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        DataTableQueryOptions tableOptions = DataTableQueryOptions.From(
            request,
            defaultSortColumn: "isDefault",
            defaultSortDirection: "desc",
            allowedSortColumns: new[] { "code", "name", "initialStatus", "isDefault", "isActive" });

        var response = await _mediator.Send(
            new GetListWorkflowTemplateQuery
            {
                Culture = culture,
        ...
        List<object> pageItems = response.Items.Select((item, index) => new
        {
            ...
            initialTransactionStatusId = item.InitialTransactionStatusId,
            initialTransactionStatusName = item.InitialTransactionStatusName,
    ...
    public async Task<IActionResult> GetForUpdate(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            GetWorkflowTemplateForUpdateResponse response = await _mediator.Send(
                new GetWorkflowTemplateForUpdateQuery { Id = id },
                cancellationToken);
            ...
            return Json(new
            {
                ...
                id = response.Id,
                code = response.Code,
                initialTransactionStatusId = response.InitialTransactionStatusId,
    ...
    public async Task<IActionResult> Create(
        [FromForm] CreateWorkflowTemplateViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await _mediator.Send(
                new CreateWorkflowTemplateCommand
                {
                    ...
                    InitialTransactionStatusId = model.InitialTransactionStatusId,
            ...
            {
                success = true,
                message = GetText("BackOffice.WorkflowTemplates.Messages.Created", "Workflow şablonu oluşturuldu.")
            });
        }
        catch (Exception exception)
        {
    ...
    public async Task<IActionResult> Update(
        [FromForm] UpdateWorkflowTemplateViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                success = true,
                message = GetText("BackOffice.WorkflowTemplates.Messages.Updated", "Workflow şablonu güncellendi.")
            });
        }
        catch (Exception exception)
        {
    ...
    public async Task<IActionResult> Delete(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await _mediator.Send(new DeleteWorkflowTemplateCommand { Id = id }, cancellationToken);

            return Json(new
            {
                success = true,
                message = GetText("BackOffice.WorkflowTemplates.Messages.Deleted", "Workflow şablonu silindi.")
            });
        }
        catch (Exception exception)
        {
    ...
    private async Task<WorkflowTemplatesIndexViewModel> BuildIndexViewModelAsync(CancellationToken cancellationToken)
    {
        string culture = await ResolveCurrentCultureAsync(cancellationToken);
        ...
        List<TransactionStatusSelectItemViewModel> statuses = await GetInitialStatusSelectItemsAsync(culture, cancellationToken);

        return new WorkflowTemplatesIndexViewModel
        {
            InitialStatuses = statuses,
            CreateWorkflowTemplate = new CreateWorkflowTemplateViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new CreateWorkflowTemplateTranslationViewModel
                {
            ...
            UpdateWorkflowTemplate = new UpdateWorkflowTemplateViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new UpdateWorkflowTemplateTranslationViewModel
                {
                    LanguageId = language.Id,
                    Culture = language.Culture,
                    LanguageName = language.Name,
    ...
    private async Task<List<TransactionStatusSelectItemViewModel>> GetInitialStatusSelectItemsAsync(
        string culture,
        CancellationToken cancellationToken)
    {
        var response = await _mediator.Send(
        ...
        return response.Items
            ...
            .Select(item => new TransactionStatusSelectItemViewModel
            {
                ...
                Code = item.Code,
                Name = item.Name,
                PhaseName = item.TransactionStatusPhaseName
            })
            .ToList();
    }
    private void ValidateCreateModel(CreateWorkflowTemplateViewModel model)
    {
        if (string.IsNullOrWhiteSpace(model.Code))
        {
            ModelState.AddModelError(nameof(model.Code), GetText("BackOffice.WorkflowTemplates.Validation.CodeRequired", "Kod alanı zorunludur."));
        }
        ...
        if (string.IsNullOrWhiteSpace(model.Translations[defaultLanguageIndex].Name))
        {
            ModelState.AddModelError(
                ...
                GetText("BackOffice.WorkflowTemplates.Validation.NameRequired", "Ad alanı zorunludur."));
        }
    }

    private void ValidateUpdateModel(UpdateWorkflowTemplateViewModel model)
    {
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(IReadOnlyCollection<CreateWorkflowTemplateTranslationViewModel> translations)
    {
        return translations
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(IReadOnlyCollection<UpdateWorkflowTemplateTranslationViewModel> translations)
    {
        return translations
            .GroupBy(translation => translation.LanguageId)
            .Select(group => group.First())
    ...
    private static Dictionary<string, string?> BuildTranslationFields(CreateWorkflowTemplateTranslationViewModel translation)
    {
    ...
    private static Dictionary<string, string?> BuildTranslationFields(UpdateWorkflowTemplateTranslationViewModel translation)
    {
        return new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
        {
            [NameField] = NormalizeText(translation.Name),
    ...
    private static bool HasAnyTranslationValue(CreateWorkflowTemplateTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) || !string.IsNullOrWhiteSpace(translation.Description);
    }

    private static bool HasAnyTranslationValue(UpdateWorkflowTemplateTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) || !string.IsNullOrWhiteSpace(translation.Description);
    }
