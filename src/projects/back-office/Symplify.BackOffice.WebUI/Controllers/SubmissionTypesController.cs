...
using Symplify.BackOffice.Application.Features.SubmissionTypes.Commands.Reorder;
using Symplify.BackOffice.Application.Features.SubmissionTypes.Commands.Update;
using Symplify.BackOffice.Application.Features.SubmissionTypes.Queries.GetForUpdate;
using Symplify.BackOffice.Application.Features.SubmissionTypes.Queries.GetList;
using Symplify.BackOffice.Application.Services.Localization;
using Symplify.BackOffice.WebUI.Localization;
using Symplify.BackOffice.WebUI.Models.Shared.DataTables;
using Symplify.BackOffice.WebUI.Models.SubmissionTypes;

namespace Symplify.BackOffice.WebUI.Controllers;

[Authorize]
[Route("{culture=tr-TR}/[controller]/[action]")]
public sealed class SubmissionTypesController : Controller
{
    ...
    {
        SubmissionTypesIndexViewModel model = await BuildIndexViewModelAsync(cancellationToken);
        return View(model);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var response = await _mediator.Send(
            new GetListSubmissionTypeQuery
            {
                Culture = culture,
    ...
    public async Task<IActionResult> GetForUpdate(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            GetSubmissionTypeForUpdateResponse response = await _mediator.Send(
                new GetSubmissionTypeForUpdateQuery
                {
                    Id = id
                },
                cancellationToken);
    ...
    public async Task<IActionResult> Create(
        [FromForm] CreateSubmissionTypeViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            CreateSubmissionTypeCommand command = new()
            {
    ...
    public async Task<IActionResult> Update(
        [FromForm] UpdateSubmissionTypeViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            UpdateSubmissionTypeCommand command = new()
            {
                Id = model.Id,
                Code = NormalizeText(model.Code),
                IsActive = model.IsActive,
    ...
    public async Task<IActionResult> Delete(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await _mediator.Send(
                new DeleteSubmissionTypeCommand
                {
                    Id = id
                },
                cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await _mediator.Send(
                new ReorderSubmissionTypeCommand
                {
                    Items = request.Items
                        .Where(item => item.Id != Guid.Empty)
                        .Select(item => new ReorderSubmissionTypeItemDto
                        {
                            Id = item.Id,
                            Order = item.Order
                        })
    ...
    private async Task<SubmissionTypesIndexViewModel> BuildIndexViewModelAsync(CancellationToken cancellationToken)
    {
        ...
        return new SubmissionTypesIndexViewModel
        {
            CreateSubmissionType = new CreateSubmissionTypeViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new CreateSubmissionTypeTranslationViewModel
                {
            ...
            UpdateSubmissionType = new UpdateSubmissionTypeViewModel
            {
                IsActive = true,
                Translations = orderedLanguages.Select(language => new UpdateSubmissionTypeTranslationViewModel
                {
                    LanguageId = language.Id,
                    Culture = language.Culture,
                    LanguageName = language.Name,
    ...
    private void ValidateCreateModel(CreateSubmissionTypeViewModel model)
    {
        ...
        CreateSubmissionTypeTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];

        if (string.IsNullOrWhiteSpace(defaultTranslation.Name))
        {
            ModelState.AddModelError(
                $"Translations[{defaultLanguageIndex}].Name",
                GetText("BackOffice.SubmissionTypes.Validation.NameRequired", "Ad alanı zorunludur."));
        }
    }

    private void ValidateUpdateModel(UpdateSubmissionTypeViewModel model)
    {
        ...
        UpdateSubmissionTypeTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<CreateSubmissionTypeTranslationViewModel> translations)
    {
        return translations
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<UpdateSubmissionTypeTranslationViewModel> translations)
    {
        return translations
            .GroupBy(translation => translation.LanguageId)
            .Select(group => group.First())
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        CreateSubmissionTypeTranslationViewModel translation)
    {
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        UpdateSubmissionTypeTranslationViewModel translation)
    {
        return new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
        {
            [NameField] = NormalizeText(translation.Name),
    ...
    private static bool HasAnyTranslationValue(CreateSubmissionTypeTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }

    private static bool HasAnyTranslationValue(UpdateSubmissionTypeTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }
