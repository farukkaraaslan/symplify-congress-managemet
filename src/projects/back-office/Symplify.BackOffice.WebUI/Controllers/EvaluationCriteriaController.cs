...
using Symplify.BackOffice.Application.Features.EvaluationCriteria.Commands.Reorder;
using Symplify.BackOffice.Application.Features.EvaluationCriteria.Commands.Update;
using Symplify.BackOffice.Application.Features.EvaluationCriteria.Queries.GetForUpdate;
using Symplify.BackOffice.Application.Features.EvaluationCriteria.Queries.GetList;
using Symplify.BackOffice.Application.Services.Localization;
using Symplify.BackOffice.WebUI.Localization;
using Symplify.BackOffice.WebUI.Models.Shared.DataTables;
using Symplify.BackOffice.WebUI.Models.EvaluationCriteria;

namespace Symplify.BackOffice.WebUI.Controllers;

[Authorize]
[Route("{culture=tr-TR}/[controller]/[action]")]
public sealed class EvaluationCriteriaController : Controller
{
    ...
    {
        EvaluationCriteriaIndexViewModel model = await BuildIndexViewModelAsync(cancellationToken);
        return View(model);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        DataTableQueryOptions tableOptions = DataTableQueryOptions.From(
            ...
            allowedSortColumns: new[] { "order", "name", "score", "isActive" });

        var response = await _mediator.Send(
            new GetListEvaluationCriterionQuery
            {
                Culture = culture,
    ...
    public async Task<IActionResult> GetForUpdate(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            GetEvaluationCriterionForUpdateResponse response = await _mediator.Send(
                new GetEvaluationCriterionForUpdateQuery
                {
                    Id = id
                },
                cancellationToken);
    ...
    public async Task<IActionResult> Create(
        [FromForm] CreateEvaluationCriterionViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            CreateEvaluationCriterionCommand command = new()
            {
            ...
            {
                success = true,
                message = GetText("Common.Created", "Kayıt oluşturuldu.")
            });
        }
        catch (Exception exception)
        {
    ...
    public async Task<IActionResult> Update(
        [FromForm] UpdateEvaluationCriterionViewModel model,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            UpdateEvaluationCriterionCommand command = new()
            {
                Id = model.Id,
                Code = NormalizeText(model.Code),
                Score = NormalizeScore(model.Score),
    ...
    public async Task<IActionResult> Delete(Guid id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await _mediator.Send(
                new DeleteEvaluationCriterionCommand
                {
                    Id = id
                },
                cancellationToken);
            ...
            {
                success = true,
                message = GetText("Common.Deleted", "Kayıt silindi.")
            });
        }
        catch (Exception exception)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await _mediator.Send(
                new ReorderEvaluationCriterionCommand
                {
                    Items = request.Items
                        .Where(item => item.Id != Guid.Empty)
                        .Select(item => new ReorderEvaluationCriterionItemDto
                        {
                            Id = item.Id,
                            Order = item.Order
                        })
    ...
    private async Task<EvaluationCriteriaIndexViewModel> BuildIndexViewModelAsync(CancellationToken cancellationToken)
    {
        ...
        return new EvaluationCriteriaIndexViewModel
        {
            CreateEvaluationCriterion = new CreateEvaluationCriterionViewModel
            {
                Score = 10,
                IsActive = true,
                Translations = orderedLanguages.Select(language => new CreateEvaluationCriterionTranslationViewModel
                {
            ...
            UpdateEvaluationCriterion = new UpdateEvaluationCriterionViewModel
            {
                Score = 10,
                IsActive = true,
                Translations = orderedLanguages.Select(language => new UpdateEvaluationCriterionTranslationViewModel
                {
                    LanguageId = language.Id,
                    Culture = language.Culture,
                    LanguageName = language.Name,
    ...
    private void ValidateCreateModel(CreateEvaluationCriterionViewModel model)
    {
        ...
        if (defaultLanguageIndex < 0)
        {
            ModelState.AddModelError(
                nameof(model.Translations),
                GetText("Common.InvalidRequest", "Geçersiz istek."));
            return;
        }

        if (model.Score < 1 || model.Score > 100)
        {
            ModelState.AddModelError(
                nameof(model.Score),
                GetText("BackOffice.EvaluationCriteria.Validation.ScoreRange", "Puan 1 ile 100 arasında olmalıdır."));
        }

        CreateEvaluationCriterionTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];

        if (string.IsNullOrWhiteSpace(defaultTranslation.Name))
        {
            ModelState.AddModelError(
                $"Translations[{defaultLanguageIndex}].Name",
                GetText("BackOffice.EvaluationCriteria.Validation.NameRequired", "Ad alanı zorunludur."));
        }
    }

    private void ValidateUpdateModel(UpdateEvaluationCriterionViewModel model)
    {
        ...
        UpdateEvaluationCriterionTranslationViewModel defaultTranslation = model.Translations[defaultLanguageIndex];
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<CreateEvaluationCriterionTranslationViewModel> translations)
    {
        return translations
    ...
    private ICollection<TranslationInputDto> BuildTranslationInputs(
        IReadOnlyCollection<UpdateEvaluationCriterionTranslationViewModel> translations)
    {
        return translations
            .GroupBy(translation => translation.LanguageId)
            .Select(group => group.First())
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        CreateEvaluationCriterionTranslationViewModel translation)
    {
    ...
    private static Dictionary<string, string?> BuildTranslationFields(
        UpdateEvaluationCriterionTranslationViewModel translation)
    {
        return new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
        {
            [NameField] = NormalizeText(translation.Name),
    ...
    private static bool HasAnyTranslationValue(CreateEvaluationCriterionTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }

    private static bool HasAnyTranslationValue(UpdateEvaluationCriterionTranslationViewModel translation)
    {
        return !string.IsNullOrWhiteSpace(translation.Name) ||
               !string.IsNullOrWhiteSpace(translation.Description);
    }
