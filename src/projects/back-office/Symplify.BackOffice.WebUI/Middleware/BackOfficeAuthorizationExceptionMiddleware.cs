...
public sealed class BackOfficeAuthorizationExceptionMiddleware
{
    ...
    private const string AccessDeniedMessageKey = "BackOffice.Auth.AccessDenied.Message";
    private const string AuthenticationRequiredMessageKey = "BackOffice.Auth.AuthenticationRequired.Message";
    ...
    private static async Task WriteJsonAuthorizationResponseAsync(HttpContext context)
    {
        ...
        string message = isAuthenticated
            ? GetLocalizedMessage(context, AccessDeniedMessageKey)
            : GetLocalizedMessage(context, AuthenticationRequiredMessageKey);
    ...
    private static string GetLocalizedMessage(HttpContext context, string key)
    {
        IBackOfficeResourceProvider resourceProvider = context.RequestServices.GetRequiredService<IBackOfficeResourceProvider>();
        return resourceProvider.GetStringValue(key, ResolveCulture(context));
    }
}
