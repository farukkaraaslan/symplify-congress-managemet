...
namespace Symplify.BackOffice.Application.Features.ProgramManagement.Commands.ReorderItems;

public sealed class ReorderProgramItemsCommand : IRequest, ISecuredRequest
{
    ...
    public sealed class Handler : IRequestHandler<ReorderProgramItemsCommand>
    {
        ...
        public async Task Handle(ReorderProgramItemsCommand request, CancellationToken cancellationToken)
        {
            ...
            List<CongressProgramSession> sessions = plan.Days.SelectMany(x => x.Sessions).ToList();
            CongressProgramSession sourceSession = sessions.FirstOrDefault(x => x.Items.Any(i => i.Id == request.MovedItemId))
                ?? throw new InvalidOperationException("Taşınacak bildiri programda bulunamadı.");
            CongressProgramSession targetSession = sessions.FirstOrDefault(x => x.Id == request.TargetSessionId)
                ?? throw new InvalidOperationException("Hedef oturum bulunamadı.");
            CongressProgramItem item = sourceSession.Items.First(x => x.Id == request.MovedItemId);
            ...
            HashSet<Guid> targetIds = targetSession.Items.Select(x => x.Id).ToHashSet();
            if (request.OrderedItemIds.Count != targetIds.Count || request.OrderedItemIds.Any(x => !targetIds.Contains(x)))
                throw new InvalidOperationException("Gönderilen sıralama listesi hedef oturumla eşleşmiyor.");

            Dictionary<Guid, int> orderMap = request.OrderedItemIds
                .Select((id, index) => new { id, order = index + 1 })
                .ToDictionary(x => x.id, x => x.order);
            foreach (CongressProgramItem targetItem in targetSession.Items)
                targetItem.Order = orderMap[targetItem.Id];
            ...
            IReadOnlyCollection<CongressProgramSession> allSessions)
        {
            ...
            foreach (CongressProgramSession session in allSessions)
            {
                ...
                foreach (CongressProgramItem otherItem in session.Items)
                {
                    if (otherItem.Id == movingItem.Id)
                        continue;

                    if (otherItem.Submission.Authors.Select(BuildAuthorKey).Any(movingAuthorKeys.Contains))
                        return true;
                }
            }
...
public sealed class ReorderProgramItemsCommandValidator : AbstractValidator<ReorderProgramItemsCommand>
{
    public ReorderProgramItemsCommandValidator()
    {
        RuleFor(x => x.CongressId).NotEmpty();
        RuleFor(x => x.MovedItemId).NotEmpty();
