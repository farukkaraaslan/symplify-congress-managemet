...
public class GetListWorkflowTemplateTransitionQuery : IRequest<IList<GetListWorkflowTemplateTransitionListItemDto>>, ISecuredRequest, ICachableRequest
{
    ...
    public class GetListWorkflowTemplateTransitionQueryHandler : IRequestHandler<GetListWorkflowTemplateTransitionQuery, IList<GetListWorkflowTemplateTransitionListItemDto>>
    {
        ...
        public async Task<IList<GetListWorkflowTemplateTransitionListItemDto>> Handle(GetListWorkflowTemplateTransitionQuery request, CancellationToken cancellationToken)
        {
            ...
            List<WorkflowTemplateTransition> items = _repository.Query()
                .Where(entity => entity.WorkflowTemplateId == request.WorkflowTemplateId && entity.IsActive)
            ...
            List<TransactionStatusTransition> transitions = transitionIds.Count == 0
                ? new()
                : _transitionRepository.Query().Where(entity => transitionIds.Contains(entity.Id) && entity.IsActive).ToList();
            ...
            Dictionary<int, TransactionStatusTransition> transitionMap = transitions.ToDictionary(entity => entity.Id);

            return items.Where(item => transitionMap.ContainsKey(item.TransactionStatusTransitionId)).Select(item =>
            {
                TransactionStatusTransition transition = transitionMap[item.TransactionStatusTransitionId];
                ...
                TransactionStatusTranslation? fromStatus = _fallbackResolver.Resolve(statusTranslations.Where(x => x.TransactionStatusId == transition.FromStatusId).ToList(), requestedLanguage.Id, defaultLanguage.Id);
                TransactionStatusTranslation? toStatus = _fallbackResolver.Resolve(statusTranslations.Where(x => x.TransactionStatusId == transition.ToStatusId).ToList(), requestedLanguage.Id, defaultLanguage.Id);
