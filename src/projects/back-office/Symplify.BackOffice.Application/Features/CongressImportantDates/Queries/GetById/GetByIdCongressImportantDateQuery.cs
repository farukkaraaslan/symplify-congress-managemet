...
public class GetByIdCongressImportantDateQuery : IRequest<GetByIdCongressImportantDateResponse>, ISecuredRequest
{
    ...
    public class GetByIdCongressImportantDateQueryHandler : IRequestHandler<GetByIdCongressImportantDateQuery, GetByIdCongressImportantDateResponse>
    {
        ...
        public GetByIdCongressImportantDateQueryHandler(
        ...
        public async Task<GetByIdCongressImportantDateResponse> Handle(
            GetByIdCongressImportantDateQuery request,
            CancellationToken cancellationToken)
        {
            ...
            ApplicationLanguageDto requestedLanguage = request.LanguageId.HasValue
                ? await _languageProvider.GetByIdAsync(request.LanguageId.Value, cancellationToken) ?? defaultLanguage
                : !string.IsNullOrWhiteSpace(request.Culture)
                    ...
                    : await _currentLanguageProvider.GetCurrentLanguageAsync(cancellationToken);

            List<CongressImportantDateTranslation> translations = _translationRepository.Query()
                .ToList()
                .Where(x => EqualityComparer<Guid>.Default.Equals(x.CongressImportantDateId, request.Id))
                .ToList();
            ...
            return new GetByIdCongressImportantDateResponse
            {
                Id = entity.Id,
