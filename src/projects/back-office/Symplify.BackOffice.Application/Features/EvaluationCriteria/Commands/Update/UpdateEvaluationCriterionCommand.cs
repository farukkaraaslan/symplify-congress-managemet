...
public class UpdateEvaluationCriterionCommand : IRequest<UpdatedEvaluationCriterionResponse>, ISecuredRequest, ICacheRemoverRequest
{
    ...
    public class UpdateEvaluationCriterionCommandHandler : IRequestHandler<UpdateEvaluationCriterionCommand, UpdatedEvaluationCriterionResponse>
    {
        ...
        public UpdateEvaluationCriterionCommandHandler(
        ...
        public async Task<UpdatedEvaluationCriterionResponse> Handle(UpdateEvaluationCriterionCommand request, CancellationToken cancellationToken)
        {
            ...
            EvaluationCriterion updatedEntity = await _repository.UpdateAsync(entity);
            await UpsertTranslationsAsync(request.Id, request.Translations, cancellationToken);

            return _mapper.Map<UpdatedEvaluationCriterionResponse>(updatedEntity);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            List<EvaluationCriterionTranslation> existingTranslations = _translationRepository.Query()
                .ToList()
                .Where(translation => EqualityComparer<Guid>.Default.Equals(translation.EvaluationCriterionId, rootId) && !IsDeleted(translation))
                .ToList();
            ...
            foreach (TranslationInputDto input in translations)
            {
                ...
                EvaluationCriterionTranslation? existingTranslation = existingTranslations
                    .FirstOrDefault(translation => translation.LanguageId == input.LanguageId);

                if (existingTranslation is null)
                {
                    EvaluationCriterionTranslation translation = new();
                    LocalizedEntityRuntimeHelper.SetPropertyValue(translation, "Id", Guid.NewGuid());
                    LocalizedEntityRuntimeHelper.SetPropertyValue(translation, "EvaluationCriterionId", rootId);
        ...
        private async Task NormalizeVisibleOrdersAsync(
            EvaluationCriterion currentEntity,
            int requestedOrder,
            CancellationToken cancellationToken)
        {
