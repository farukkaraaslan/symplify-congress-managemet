...
public sealed class SaveEditorEvaluationCommand : IRequest<SavedEditorEvaluationResponse>, ISecuredRequest, ICacheRemoverRequest
{
    ...
    public sealed class Handler : IRequestHandler<SaveEditorEvaluationCommand, SavedEditorEvaluationResponse>
    {
        ...
        public async Task<SavedEditorEvaluationResponse> Handle(SaveEditorEvaluationCommand request, CancellationToken cancellationToken)
        {
            ...
            decimal totalScore = normalizedScores.Sum(score => score.Score!.Value);
            ...
            if (IsAcceptanceRecommendation(evaluation.Recommendation) && await CanAutoAcceptSubmissionAsync(evaluation.SubmissionId, cancellationToken))
            {
                ChangeWorkflowStatusResult workflowResult = await _workflowEngine.ExecuteAutomaticTransitionToStatusAsync(
                    ...
                    "Editör değerlendirmesi olumlu tamamlandı. Kabul mektubu oluşturulacak.",
                    evaluation.EditorComment,
                    cancellationToken);

                if (!workflowResult.Success)
                    throw new BusinessException(workflowResult.Message ?? SubmissionsMessages.EvaluationStatusTransitionFailed);
            }

            return new SavedEditorEvaluationResponse
            {
