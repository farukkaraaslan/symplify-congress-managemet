...
public class GetCongressWorkflowByCongressIdQuery : IRequest<GetCongressWorkflowByCongressIdResponse>, ISecuredRequest, ICachableRequest
{
    ...
    public string CacheKey => $"GetCongressWorkflow({CongressId},{LanguageId},{Culture})";
    public string CacheGroupKey => $"GetCongressWorkflow({CongressId})";
    public TimeSpan? SlidingExpiration { get; }

    public class GetCongressWorkflowByCongressIdQueryHandler : IRequestHandler<GetCongressWorkflowByCongressIdQuery, GetCongressWorkflowByCongressIdResponse>
    {
        private readonly ICongressWorkflowSettingRepository _settingRepository;
        ...
        public GetCongressWorkflowByCongressIdQueryHandler(
            ICongressWorkflowSettingRepository settingRepository,
        ...
        public async Task<GetCongressWorkflowByCongressIdResponse> Handle(GetCongressWorkflowByCongressIdQuery request, CancellationToken cancellationToken)
        {
            ...
            CongressWorkflowSetting? setting = await _settingRepository.GetAsync(predicate: x => x.CongressId == request.CongressId, cancellationToken: cancellationToken);

            List<CongressTransactionStatusTransition> congressTransitions = _congressTransitionRepository.Query().ToList().Where(entity => entity.CongressId == request.CongressId).OrderBy(entity => entity.Order).ThenBy(entity => entity.Id).ToList();
            HashSet<int> transitionIds = congressTransitions.Select(entity => entity.TransactionStatusTransitionId).ToHashSet();
            ...
            if (setting?.InitialTransactionStatusId is not null)
                statusIds.Add(setting.InitialTransactionStatusId.Value);
            ...
            if (setting?.InitialTransactionStatusId is not null)
            {
                TransactionStatusTranslation? displayInitialStatus = _fallbackResolver.Resolve(statusTranslations.Where(x => x.TransactionStatusId == setting.InitialTransactionStatusId.Value).ToList(), requestedLanguage.Id, defaultLanguage.Id);
                initialStatusName = displayInitialStatus?.Name;
            }

            return new GetCongressWorkflowByCongressIdResponse
            {
                CongressId = request.CongressId,
                SourceWorkflowTemplateId = setting?.SourceWorkflowTemplateId,
                InitialTransactionStatusId = setting?.InitialTransactionStatusId,
                InitialTransactionStatusName = initialStatusName,
                IsActive = setting?.IsActive ?? false,
                Transitions = congressTransitions.Select(item =>
                {
                    ...
                    return new CongressWorkflowTransitionListItemDto
                    {
                        Id = item.Id,
