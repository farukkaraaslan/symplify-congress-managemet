...
public class UpdateSubmissionTypeCommand : IRequest<UpdatedSubmissionTypeResponse>, ISecuredRequest, ICacheRemoverRequest
{
    ...
    public class UpdateSubmissionTypeCommandHandler : IRequestHandler<UpdateSubmissionTypeCommand, UpdatedSubmissionTypeResponse>
    {
        ...
        public UpdateSubmissionTypeCommandHandler(
        ...
        public async Task<UpdatedSubmissionTypeResponse> Handle(UpdateSubmissionTypeCommand request, CancellationToken cancellationToken)
        {
            ...
            SubmissionType updatedEntity = await _repository.UpdateAsync(entity);
            await UpsertTranslationsAsync(request.Id, request.Translations, cancellationToken);

            return _mapper.Map<UpdatedSubmissionTypeResponse>(updatedEntity);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            List<SubmissionTypeTranslation> existingTranslations = _translationRepository.Query()
                .ToList()
                .Where(translation => EqualityComparer<Guid>.Default.Equals(translation.SubmissionTypeId, rootId) && !IsDeleted(translation))
                .ToList();
            ...
            foreach (TranslationInputDto input in translations)
            {
                ...
                SubmissionTypeTranslation? existingTranslation = existingTranslations
                    .FirstOrDefault(translation => translation.LanguageId == input.LanguageId);

                if (existingTranslation is null)
                {
                    SubmissionTypeTranslation translation = new();
                    LocalizedEntityRuntimeHelper.SetPropertyValue(translation, "Id", Guid.NewGuid());
                    LocalizedEntityRuntimeHelper.SetPropertyValue(translation, "SubmissionTypeId", rootId);
