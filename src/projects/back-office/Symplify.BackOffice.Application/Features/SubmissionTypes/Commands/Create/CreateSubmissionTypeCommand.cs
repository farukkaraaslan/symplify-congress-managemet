...
public class CreateSubmissionTypeCommand : IRequest<CreatedSubmissionTypeResponse>, ISecuredRequest, ICacheRemoverRequest
{
    ...
    public string[] Roles => new[] { SubmissionTypesOperationClaims.Admin, SubmissionTypesOperationClaims.Write, SubmissionTypesOperationClaims.Add };

    public class CreateSubmissionTypeCommandHandler : IRequestHandler<CreateSubmissionTypeCommand, CreatedSubmissionTypeResponse>
    {
        ...
        public CreateSubmissionTypeCommandHandler(
        ...
        public async Task<CreatedSubmissionTypeResponse> Handle(CreateSubmissionTypeCommand request, CancellationToken cancellationToken)
        {
            await _rules.DefaultTranslationShouldExist(request.Translations, cancellationToken);

            SubmissionType entity = new()
            {
                Id = Guid.NewGuid(),
                Code = request.Code,
                Order = 0,
            ...
            SubmissionType createdEntity = await _repository.AddAsync(entity);
            ...
            return _mapper.Map<CreatedSubmissionTypeResponse>(createdEntity);
        }
