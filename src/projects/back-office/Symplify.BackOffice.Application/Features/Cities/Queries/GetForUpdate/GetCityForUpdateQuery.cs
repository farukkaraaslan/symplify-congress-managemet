...
public class GetCityForUpdateQuery : IRequest<GetCityForUpdateResponse>, ISecuredRequest
{
    public Guid Id { get; set; }
    public string[] Roles => new[] { CitiesOperationClaims.Admin, CitiesOperationClaims.Read };
    public class GetCityForUpdateQueryHandler : IRequestHandler<GetCityForUpdateQuery, GetCityForUpdateResponse>
    {
        private static readonly string[] TranslationFieldNames = new[] { "Name" };
        private readonly ICityRepository _repository; private readonly ICityTranslationRepository _translationRepository; private readonly IApplicationLanguageProvider _languageProvider;
        public GetCityForUpdateQueryHandler(ICityRepository repository, ICityTranslationRepository translationRepository, IApplicationLanguageProvider languageProvider) { _repository = repository; _translationRepository = translationRepository; _languageProvider = languageProvider; }
        public async Task<GetCityForUpdateResponse> Handle(GetCityForUpdateQuery request, CancellationToken cancellationToken)
        {
