...
public class CreateEventRoomCommand : IRequest<CreatedEventRoomResponse>, ISecuredRequest, ICacheRemoverRequest
{
    ...
    public string[] Roles => new[] { EventRoomsOperationClaims.Admin, EventRoomsOperationClaims.Write, EventRoomsOperationClaims.Add };

    public class CreateEventRoomCommandHandler : IRequestHandler<CreateEventRoomCommand, CreatedEventRoomResponse>
    {
        ...
        public CreateEventRoomCommandHandler(
        ...
        public async Task<CreatedEventRoomResponse> Handle(CreateEventRoomCommand request, CancellationToken cancellationToken)
        {
            await _rules.DefaultTranslationShouldExist(request.Translations, cancellationToken);

            EventRoom entity = new()
            {
                Id = Guid.NewGuid(),
                Code = request.Code,
                Order = 0,
            ...
            EventRoom createdEntity = await _repository.AddAsync(entity);
            ...
            return _mapper.Map<CreatedEventRoomResponse>(createdEntity);
        }
