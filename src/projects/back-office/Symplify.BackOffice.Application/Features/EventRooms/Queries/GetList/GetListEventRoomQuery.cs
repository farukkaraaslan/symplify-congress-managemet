...
namespace Symplify.BackOffice.Application.Features.EventRooms.Queries.GetList;

public class GetListEventRoomQuery : IRequest<GetListResponse<GetListEventRoomListItemDto>>, ISecuredRequest, ICachableRequest
{
    ...
    public string CacheKey => $"GetListEventRooms({PageRequest.Page},{PageRequest.PageSize},{LanguageId},{Culture},{IsActive},{SearchText},{SortColumn},{SortDirection})";

    public string CacheGroupKey => "GetEventRooms";

    public TimeSpan? SlidingExpiration { get; }

    public class GetListEventRoomQueryHandler : IRequestHandler<GetListEventRoomQuery, GetListResponse<GetListEventRoomListItemDto>>
    {
        ...
        public GetListEventRoomQueryHandler(
        ...
        public async Task<GetListResponse<GetListEventRoomListItemDto>> Handle(
            GetListEventRoomQuery request,
            CancellationToken cancellationToken)
        {
            ...
            IQueryable<EventRoomTranslation> translationQuery = _translationRepository.Query();

            IPaginate<EventRoom> roots = await _repository.GetListAsync(
            ...
            List<EventRoomTranslation> translations = ids.Count == 0
                ? new List<EventRoomTranslation>()
                : translationQuery.Where(translation => ids.Contains(translation.EventRoomId)).ToList();

            List<GetListEventRoomListItemDto> items = roots.Items.Select(entity =>
            {
                List<EventRoomTranslation> rootTranslations = translations
                    .Where(translation => EqualityComparer<Guid>.Default.Equals(translation.EventRoomId, entity.Id))
                    .ToList();

                EventRoomTranslation? requestedTranslation = rootTranslations.FirstOrDefault(translation => translation.LanguageId == requestedLanguage.Id);
                EventRoomTranslation? displayTranslation = _fallbackResolver.Resolve(rootTranslations, requestedLanguage.Id, defaultLanguage.Id);

                return new GetListEventRoomListItemDto
                {
                    Id = entity.Id,
                    Code = entity.Code,
                    Order = entity.Order,
            ...
            return new GetListResponse<GetListEventRoomListItemDto>
            {
                Index = roots.Index,
                Size = roots.Size,
                Count = roots.Count,
        ...
        private static Expression<Func<EventRoom, bool>>? BuildPredicate(
            GetListEventRoomQuery request,
            IQueryable<EventRoomTranslation> translationQuery)
        {
            ...
            IQueryable<Guid> matchingTranslationRootIds = translationQuery
                ...
                .Select(translation => translation.EventRoomId);
        ...
        private static Func<IQueryable<EventRoom>, IOrderedQueryable<EventRoom>> BuildOrderBy(
            string? sortColumn,
                ...
                "name" => query => descending
                    ? query
                        .OrderByDescending(entity =>
                            translationQuery
                                .Where(translation => translation.EventRoomId == entity.Id && translation.LanguageId == requestedLanguageId)
                                .Select(translation => translation.Name)
                                .FirstOrDefault()
                            ?? translationQuery
                                .Where(translation => translation.EventRoomId == entity.Id && translation.LanguageId == defaultLanguageId)
                                .Select(translation => translation.Name)
                                .FirstOrDefault()
                            ?? string.Empty)
