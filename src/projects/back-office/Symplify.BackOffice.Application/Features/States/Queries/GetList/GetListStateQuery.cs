...
namespace Symplify.BackOffice.Application.Features.States.Queries.GetList;
public class GetListStateQuery : IRequest<GetListResponse<GetListStateListItemDto>>, ISecuredRequest, ICachableRequest
{
    ...
    public string CacheKey => $"GetListStates({PageRequest.Page},{PageRequest.PageSize},{LanguageId},{Culture},{IsActive})";
    public string CacheGroupKey => "GetStates";
    public TimeSpan? SlidingExpiration { get; }
    public class GetListStateQueryHandler : IRequestHandler<GetListStateQuery, GetListResponse<GetListStateListItemDto>>
    {
        ...
        public async Task<GetListResponse<GetListStateListItemDto>> Handle(GetListStateQuery request, CancellationToken cancellationToken)
        {
            ...
            List<State> roots = _repository.Query().ToList();
            ...
            List<State> paged = roots.Skip(page * pageSize).Take(pageSize).ToList();
            HashSet<Guid> ids = paged.Select(x => x.Id).ToHashSet();
            List<StateTranslation> translations = _translationRepository.Query().ToList().Where(x => ids.Contains(x.StateId)).ToList();
            List<GetListStateListItemDto> items = paged.Select(entity =>
            {
                List<StateTranslation> rootTranslations = translations.Where(x => EqualityComparer<Guid>.Default.Equals(x.StateId, entity.Id)).ToList();
                StateTranslation? requestedTranslation = rootTranslations.FirstOrDefault(x => x.LanguageId == requestedLanguage.Id);
                StateTranslation? displayTranslation = _fallbackResolver.Resolve(rootTranslations, requestedLanguage.Id, defaultLanguage.Id);
                return new GetListStateListItemDto
                {
                    Id = entity.Id,
            ...
            return new GetListResponse<GetListStateListItemDto> { Index = page, Size = pageSize, Count = total, Pages = pages, HasPrevious = page > 0, HasNext = page + 1 < pages, Items = items };
        }
