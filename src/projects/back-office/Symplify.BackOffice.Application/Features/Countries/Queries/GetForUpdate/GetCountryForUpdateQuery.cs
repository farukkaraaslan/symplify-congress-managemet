...
public class GetCountryForUpdateQuery : IRequest<GetCountryForUpdateResponse>, ISecuredRequest
{
    public Guid Id { get; set; }
    public string[] Roles => new[] { CountriesOperationClaims.Admin, CountriesOperationClaims.Read };
    public class GetCountryForUpdateQueryHandler : IRequestHandler<GetCountryForUpdateQuery, GetCountryForUpdateResponse>
    {
        private static readonly string[] TranslationFieldNames = new[] { "Name" };
        private readonly ICountryRepository _repository; private readonly ICountryTranslationRepository _translationRepository; private readonly IApplicationLanguageProvider _languageProvider;
        public GetCountryForUpdateQueryHandler(ICountryRepository repository, ICountryTranslationRepository translationRepository, IApplicationLanguageProvider languageProvider) { _repository = repository; _translationRepository = translationRepository; _languageProvider = languageProvider; }
        public async Task<GetCountryForUpdateResponse> Handle(GetCountryForUpdateQuery request, CancellationToken cancellationToken)
        {
