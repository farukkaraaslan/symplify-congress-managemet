...
namespace Symplify.BackOffice.Application.Features.AbstractBook.Queries.GetPdf;

public sealed class GetAbstractBookPdfQuery : IRequest<AbstractBookFileResponse>, ISecuredRequest
{
    ...
    public sealed class Handler : IRequestHandler<GetAbstractBookPdfQuery, AbstractBookFileResponse>
    {
        private readonly IAbstractBookDocumentBuilder _builder;
        private readonly IAbstractBookPdfRenderer _renderer;

        public Handler(
            IAbstractBookDocumentBuilder builder,
            IAbstractBookPdfRenderer renderer)
        {
            _builder = builder;
            _renderer = renderer;
        }
        ...
        public async Task<AbstractBookFileResponse> Handle(
            GetAbstractBookPdfQuery request,
            CancellationToken cancellationToken)
        {
            ...
            return new AbstractBookFileResponse(content, BuildFileName(model.CongressName, "pdf"));
        }
    }

    private static string BuildFileName(string congressName, string extension)
    {
        ...
        return $"{safe}-ozet-kitabi.{extension}";
    }
}
