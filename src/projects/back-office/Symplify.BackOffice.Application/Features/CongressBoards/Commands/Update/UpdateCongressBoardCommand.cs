...
public class UpdateCongressBoardCommand : IRequest<UpdatedCongressBoardResponse>, ISecuredRequest, ICacheRemoverRequest
{
    ...
    public class UpdateCongressBoardCommandHandler : IRequestHandler<UpdateCongressBoardCommand, UpdatedCongressBoardResponse>
    {
        ...
        public async Task<UpdatedCongressBoardResponse> Handle(UpdateCongressBoardCommand request, CancellationToken cancellationToken)
        {
            ...
            List<CongressBoardTranslation> existingTranslations = _translationRepository.Query().ToList().Where(x => EqualityComparer<Guid>.Default.Equals(x.CongressBoardId, request.Id)).ToList();
            foreach (TranslationInputDto input in request.Translations)
            {
                ...
                CongressBoardTranslation? existingTranslation = existingTranslations.FirstOrDefault(x => x.LanguageId == input.LanguageId);
                if (existingTranslation is null)
                {
                    CongressBoardTranslation translation = new();
                    LocalizedEntityRuntimeHelper.SetPropertyValue(translation, "Id", Guid.NewGuid());
                    LocalizedEntityRuntimeHelper.SetPropertyValue(translation, "CongressBoardId", request.Id);
